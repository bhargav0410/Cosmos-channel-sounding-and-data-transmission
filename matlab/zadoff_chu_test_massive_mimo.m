...
for mod_size = [2]
    num_ofdm_syms = ceil(1e5/((log2(mod_size))*(NFFT-1)));
num_samps = 2047;

zc_seq(:,1) = exp(-1i*((pi*1*(0:num_samps-1).*((0:num_samps-1) + 1))./(num_samps)));


for rx_chan = [1]
for tx_chan = [1 2 4 8 16]
...
%         num2str(Fs/1e6),'mhz_',num2str(NFFT),'subcar_',num2str(rx_chan),'ants_',h.FadingDistribution,'_single_zc_seq_ber.txt'];
%     file1 = fopen(DIR,'w');
%     DIR = ['D:\Cosmos-channel-sounding-and-data-transmission\',num2str(mod_size),'qam_', ...
%         num2str(Fs/1e6),'mhz_',num2str(NFFT),'subcar_',num2str(rx_chan),'ants_',h.FadingDistribution,'_single_zc_seq_mse.txt'];
%     file2 = fopen(DIR,'w');
%     DIR = ['D:\Cosmos-channel-sounding-and-data-transmission\',num2str(mod_size),'qam_', ...
%         num2str(Fs/1e6),'mhz_',num2str(NFFT),'subcar_',num2str(rx_chan),'ants_',h.FadingDistribution,'_single_zc_seq_mse_td.txt'];
%     file3 = fopen(DIR,'w');
...
%     corr_val = [];
%     corr_thres = [];
%     corr_val = abs(corr(zc_seq(:,1:sim_tx))) - eye(sim_tx);
...
input = reshape(qammod(in_len,mod_size, 'UnitAveragePower', true),NFFT - 1,ofdm_mod.NumSymbols);
        ...
        for i = 1:tx_chan
            ...
            for j = 1:tx_chan
                if i == j
                    pn_test(:,i) = [1;zeros(num_samps-1,1)];%zc_seq(:,1);
                end
            end
            mse_test(:,i) = h(pn_test);
        end
        % for i = 1:ofdm_mod.CyclicPrefixLength
        %     C_mat(i,:) = circshift(zc_seq(:,1),i-1);
        % end
...
for snr = 0:50
    ...
    for total_tx = 0:sim_tx:h.NumTransmitAntennas-1
        pil = [];
        for tx = 1:sim_tx
%             pil(:,tx) = [zc_seq(num_samps - cp_len-1:num_samps-1,tx);zc_seq(:,tx)];
            pil(:,tx) = [circshift(zc_seq(num_samps - cp_len-1:num_samps-1),(tx-1)*floor(num_samps/sim_tx)); ...
                circshift(zc_seq(:,1),(tx-1)*floor(num_samps/sim_tx))];
...
%             in_seq(:,tx + total_tx - 1) = in_seq(:,tx + total_tx - 1)/max(abs(in_seq(:,tx + total_tx - 1)));
    ...
    y = awgn(h(repmat(in_seq,100,1)),snr,'measured');
% y_test = h([1; zeros(NFFT - 1-1,1)]);

% plot(abs(fft(xcorr(zc_seq,zc_seq) + xcorr(zc_seq,zc_seq_2) + xcorr(zc_seq,zc_seq_3),1024))/(NFFT - 1+1));
        ...
        for rx = 1:rx_chan
            for total_tx = 0:sim_tx:tx_chan-1
                ...
                if sim_tx == 1
                    ...
                    H_est(:,tx+total_tx) = C_mat'*y((num_samps + cp_len+1)*(total_tx/sim_tx) + cp_len+2 + reps*length(in_seq):(num_samps + cp_len + 1)*((total_tx/sim_tx) + 1) + reps*length(in_seq),rx)/num_samps;
    %                 thres = 0.1;
    %                 for i = 1:length(H_est)
    %                     if abs(H_est(i,tx+total_tx)) <= (10^(-snr/10))/(num_samps)
    ...
    %                 end
                    ...
                    for tx = 1:sim_tx
                        for i = 1:ofdm_mod.CyclicPrefixLength
                            C_mat(:,i) = circshift((zc_seq(:,1)),(tx-1)*floor(num_samps/sim_tx)+(i-1));
                        end

                        H_est(:,tx+total_tx) = C_mat'*y((num_samps + cp_len+1)*(total_tx)/sim_tx + cp_len+2:(num_samps + cp_len + 1)*((total_tx)/sim_tx + 1),rx)/num_samps;
                        thres = 0.1;
    %                     for i = 1:length(H_est)
    %                         if abs(H_est(i)) < 0 %corr_thres(tx)
    %                             H_est(i) = 0;
    %                         end
    %                     end
                        H_fft = (fft(([H_est(:,tx+total_tx);zeros(num_samps - ofdm_mod.CyclicPrefixLength+1,1)]),ofdm_demod.FFTLength));
    ...
    H_final = (reps*H_final + H_interp)/(reps+1);
    end
...
%         pilot_out(:,tx) = [zc_seq(NFFT - cp_len-1:NFFT-1,tx);zc_seq(:,tx)];
...
%         end
    ...
    mse_td(iter) = mean(mean(abs([H_est(:,:);zeros(num_samps - ofdm_mod.CyclicPrefixLength,tx_chan)] - mse_test).^2));
...
%     fprintf(file3,'%f %f\n',[snr mse_td(iter)]);
    ...
    figure(mod_size+1); semilogy(0:50,mse, '-'); hold on;
    title([num2str(mod_size),'-QAM, ',num2str(Fs/1e6),' MHz BW, ',num2str(NFFT),' sub-carriers, and ',num2str(tx_chan),' ants']);
    grid on;
    xlabel('SNR (dB)');
    ylabel('MSE');
