...
int main(int argc, char *argv[]) {
    ...
    int pn_len = 1023;
    std::vector<std::vector<int>> polynomial(4, std::vector<int>(11));
    ...
    for (int times = 0; times < num_times; times++) {
        ...
        create_ofdm_sounding_frame(pilots, out_vec, fft_size, prefix_size, num_ants, (int)(offset_ants*(fft_size + prefix_size)), (int)(offset_ants*(fft_size + prefix_size)), num_reps, num_threads);
        ...
        start = high_resolution_clock::now();
        sound_frame(pilots, chan_vec, demod_vec, fft_size, prefix_size, num_ants*num_reps, num_rx_ants, num_ants, num_threads);
